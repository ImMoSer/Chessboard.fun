...
## 7. Сценарий «Игры на рельсах» (Rails Validation)

Чтобы исключить «грязные хаки» и ручные манипуляции с PGN, внедряется строгая механика валидации **до исполнения хода**. Доска в этом режиме работает как трамвай на рельсах: свернуть невозможно.

### Механика валидации (Guard-Logic):
1.  **Пре-валидация**: Когда игрок пытается сделать ход (`validateUserMove`), система не ждет, пока ход запишется в историю. Она мгновенно сверяет UCI-строку с разрешенными ходами из Gravity API.
2.  **Запрет схода с рельс**:
    - Если хода нет в базе — `validateUserMove` возвращает `false`.
    - Визуально: фигура игрока «отпрыгивает» на место (стандартное поведение Chessground при отклонении хода).
    - Звук: `game_tacktics_error`.
    - PGN: **Остается девственно чистым**, никакой `undoLastMove` не требуется.
3.  **Разветвление (The Branching Point)**:
    - Единственный момент, когда «рельсы» меняются — это зевок бота (`NAG 4`). 
    - В этот момент старые рельсы (теория) заканчиваются, и начинаются новые — «рельсы опровержения».

### Жизненный цикл «Бриллианта» на рельсах:
- **Phase Hunt**: Можно ходить в пределах любых веток Gravity Book. Свернул не туда — фигура отпрыгнула, едем дальше.
- **Phase Solving**: Бот зевнул. Теперь рельсы — это только те ходы, которые ведут к наказанию (`NAG 3` или `NAG 255`). Попытка сделать другой ход (даже легальный по правилам шахмат) блокируется на уровне валидатора.
- **Phase Saving**: Доска сброшена. Рельсы — это массив `savingMoves`. Шаг вправо, шаг влево — блокировка хода + показ стрелки-подсказки «куда надо было».

**Итог:** Мы больше не лечим болезнь (не чистим PGN после ошибки), мы предотвращаем её (не даем совершить ошибочный ход в историю).

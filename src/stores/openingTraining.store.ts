// src/stores/openingTraining.store.ts
...
export const useOpeningTrainingStore = defineStore('openingTraining', () => {
  ...
  // Database settings - Fixed for Training
  const dbSource = ref<OpeningDatabaseSource>('lichess');
  ...
  async function processMoveQueue() {
    ...
    const maxGames = Math.max(...currentStats.value.moves.map((m: any) => m.white + m.draws + m.black));

    ...
    render(row: GravityMoveWithPercent) {
      ...
      if (row.dist <= 3) {
        return [
          row.dist,
          ' ',
          h(NTag, { type: 'success', size: 'tiny', bordered: false }, { default: () => '!' }),
        ]
      } else if (row.dist <= 5) {
        return [
          row.dist,
          ' ',
          h(NTag, { type: 'info', size: 'tiny', bordered: false }, { default: () => '!?' }),
        ]
      }
      return row.dist
    },
...
<template>
  <div class="gravity-book">
    ...
    <div class="content">
      ...
      />
      <n-empty
        v-if="!isLoading && moves.length === 0"
        description="No Gravity Data"
        style="padding: 20px"
      />
    </div>
  </div>

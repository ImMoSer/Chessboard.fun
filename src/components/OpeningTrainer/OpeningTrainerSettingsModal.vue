...
import type { LichessParams, OpeningDatabaseSource } from '../../services/OpeningApiService';
...
const localSessionMode = ref<'game' | 'training'>('training');
...
onMounted(async () => {
  ...
  localSessionMode.value = openingStore.sessionMode;
...
function startSession() {
  ...
  openingStore.sessionMode = localSessionMode.value;
...
<template>
  <n-modal :show="true" preset="card"
    :style="{ width: localDbSource === 'lichess' ? '920px' : '600px', borderRadius: '16px' }" class="settings-modal"
    ...
    <div class="modal-body-layout">
      <n-grid :cols="24" :x-gap="localDbSource === 'lichess' ? 32 : 0" :y-gap="24">
        <!-- Main Column: Primary Controls (Span 10/24 if Lichess, 24/24 otherwise) -->
        <n-grid-item :span="localDbSource === 'lichess' ? 10 : 24">
          <n-space vertical :size="24">
            ...
            <div class="setting-section">
              <n-space align="center" :size="12" class="section-title">
                ...
                <n-text strong>{{ t('openingTrainer.header.mode.label') }}</n-text>
              </n-space>
              <n-radio-group v-model:value="localSessionMode" name="sessionMode" size="large" expand>
                <n-radio-button value="training" class="mode-btn-training">
                  <n-space align="center" justify="center" :wrap="false">
                    <n-icon>
                      <SchoolOutline />
                    </n-icon>
                    {{ t('openingTrainer.header.mode.training') }}
                  </n-space>
                </n-radio-button>
                <n-radio-button value="game" class="mode-btn-game">
                  <n-space align="center" justify="center" :wrap="false">
                    <n-icon>
                      <TrophyOutline />
                    </n-icon>
                    {{ t('openingTrainer.header.mode.game') }}
                  </n-space>
                </n-radio-button>
              </n-radio-group>
            </div>
            ...
            <div class="setting-section">
              ...
              <n-radio-group v-model:value="localDbSource" name="db" size="large" expand>
        ...
        <!-- Lichess Settings Panel (Middle Area) - Only shown if Lichess DB is selected -->
        <n-grid-item v-if="localDbSource === 'lichess'" :span="14">
...
.mode-btn-training {
  --n-button-color-active: var(--color-accent-info) !important;
  --n-button-text-color-active: #fff !important;
}

.mode-btn-game {
  --n-button-color-active: var(--color-accent-warning) !important;
  --n-button-text-color-active: var(--color-text-dark) !important;
}

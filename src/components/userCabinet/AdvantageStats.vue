<!-- src/components/userCabinet/AdvantageStats.vue -->
<script setup lang="ts">
import { ref, computed } from 'vue'
import { useI18n } from 'vue-i18n'
import type { PropType } from 'vue'
import type { TimedModeStatsDto, AdvantageMode, ModeStatsDto } from '@/types/api.types'
...
type DisplayMode = AdvantageMode | 'All'

const sortKey = ref<SortKey>('rating')
const sortOrder = ref<'asc' | 'desc'>('desc')

const availableModes = computed<AdvantageMode[]>(() => {
  if (!props.stats) return []
  return Object.keys(props.stats).filter((mode) => {
    const modeStats = props.stats![mode as AdvantageMode]
...
const sortedStats = computed(() => {
  ...
  if (activeMode.value === 'All') {
    ...
    statsForMode = props.stats[activeMode.value as AdvantageMode] ?? null
  }

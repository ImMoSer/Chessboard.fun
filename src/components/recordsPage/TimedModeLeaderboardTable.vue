...
import type { TornadoLeaderboardEntry, AdvantageLeaderboardEntry, TornadoMode } from '@/types/api.types'
            ...
            <n-tabs type="segment" animated>
                <n-tab-pane v-for="modeDef in MODE_DEFINITIONS" :key="modeDef.id" :name="modeDef.id">
                    ...
                    <div class="mode-table-wrapper">
                        <n-data-table :columns="columns" :data="data[modeDef.id] || []"
                            :row-key="(row: any) => row.lichess_id" size="small" striped class="records-table"
                            :max-height="400" />
                    </div>
                </n-tab-pane>
            </n-tabs>

...
export function transformPuzzle(
  ...
  // TORNADO
  if (
    'tactical_rating' in puzzle &&
    'themes' in puzzle &&
    Array.isArray((puzzle as TornadoPuzzle).themes)
  ) {
...
function transformFinishHim(puzzle: FinishHimPuzzle, t: (key: string) => string): DisplayTask {
  return {
    ...
    stats: [
      ...
      {
        icon: 'advantage',
        value: `+${puzzle.material_advantage}`,
        label: t('chess.types.advantage'),
      },
    ],
    footerTags: [
